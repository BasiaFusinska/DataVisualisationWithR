...
user.area <- user.area[sample(1:500, 150, replace=FALSE),]
user.area <- aggregate(cbind(user.area[0],weight=1), user.area, length)
...
V(user.area.graph)$color <- c(rep("gray50", length(unique.users)), rep("gold", length(unique.areas)))
